    ...
    <div className="page animate-fade-in">
      ...
      <div className="card shadow-sm">
        <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div className="card-title"><FiFileText style={{ marginRight: '8px' }} /> Recent Transactions</div>
          ...
          <table className="table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="6" className="text-center py-4 text-secondary">No invoices found.</td></tr>
              ) : (
                filteredInvoices.map(inv => (
                  <tr key={inv.id}>
                    <td className="fw-bold text-primary">{inv.invoice_no}</td>
                    <td>{new Date(inv.invoice_date).toLocaleDateString()}</td>
                    <td>{inv.customer_name}</td>
                    <td>{inv.payment_type == 2 ? 'Credit' : 'Cash'}</td>
                    <td className="text-right font-mono fw-bold">Rs. {parseFloat(inv.grand_total).toFixed(2)}</td>
                    <td className="text-center">
                      <button className="btn btn-secondary btn-icon" onClick={() => handleView(inv.id)} title="View Details">
                        <FiEye />
                      </button>
                    </td>
                  </tr>
