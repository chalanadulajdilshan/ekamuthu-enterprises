$(document).ready(function () {
  ...
  // Select customer from modal - listen for customer-row clicks
  // (common.js sets #customer_id and #customer_code via data attributes)
  $(document).on("click", ".customer-row", function () {
    const id = $(this).data("id");
    if (id) {
      // common.js already sets #customer_id and #customer_code
      // We just need to load the outstanding items
      loadOutstandingItems(id);
    }
  });
  ...
  function renderTable(items) {
    ...
    if (items.length === 0) {
      html =
        '<tr><td colspan="6" class="text-center">No outstanding items found.</td></tr>';
    } else {
      items.forEach((item) => {
        html += `
                    <tr>
  ...
  $("#payBtn").click(function () {
    ...
    if (paymentMethod == "2") {
      ...
      if (!bankId || !branchId || !chequeNo || !chequeDate) {
        Swal.fire(
          "Error",
          "Please fill in all Cheque details (Bank, Branch, Cheque No, Date)",
          "error",
        );
        return;
      }
      ...
      if (!bankId || !branchId || !accountNo || !transferDate) {
        Swal.fire(
          "Error",
          "Please fill in all Transfer details (Bank, Branch, Account No, Date)",
          "error",
        );
        return;
      }
