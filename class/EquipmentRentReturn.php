...
class EquipmentRentReturn
{
    ...
    public static function calculateSettlement($rent_item_id, $return_qty, $damage_amount, $return_date = null, $return_time = null, $after_9am_extra_day = 0, $extra_day_amount = 0, $penalty_percentage = 0)
    {
        ...
        $rental_date_time = strtotime($rental_date);
        $days_in_rental_month = ((int)date('n', $rental_date_time) === 2) ? 30 : (int)date('t', $rental_date_time);
        $duration_days = ($rent_type === 'month') ? $duration * $days_in_rental_month : $duration;
        ...
        // For monthly items: charge minimum of days in rented month, then exact days
        // Rental = monthly_rate * charged_days * return_qty
        $used_months = 0;
        $overdue_months = 0;
        $charged_days_monthly = 0;
        $actual_datediff = max(0, (int)round(($return_dt - $rental_dt) / 86400));
        
        if ($rent_type === 'month') {
            $charged_days_monthly = max($days_in_rental_month, $actual_datediff);
            $used_months = max(1, (int)ceil($actual_datediff / $days_in_rental_month));
            
            $is_late = $actual_datediff > $duration_days;
            $overdue_months = $is_late ? max(0, $used_months - (int)$duration) : 0;
            $overdue_days = $is_late ? ($actual_datediff - $duration_days) : 0;
        } else {
        ...
        $prevRentQuery = "SELECT COALESCE(SUM(
                            CASE WHEN err2.rental_override IS NOT NULL
                                ...
                                ELSE CASE WHEN COALESCE(e2.is_fixed_rate, 0) = 1
                                    ...
                                    * GREATEST(CASE WHEN MONTH(eri2.rental_date) = 2 THEN 30 ELSE DAY(LAST_DAY(eri2.rental_date)) END, DATEDIFF(err2.return_date, eri2.rental_date))
                                        * err2.return_qty)
    ...
    public static function getByRentItemId($rent_item_id)
    {
        $query = "SELECT err.*, u.name as created_by_name,
                         ...
                         CASE WHEN eri.rent_type = 'month' THEN eri.duration * (CASE WHEN MONTH(eri.rental_date) = 2 THEN 30 ELSE DAY(LAST_DAY(eri.rental_date)) END) ELSE eri.duration END AS duration_days,
